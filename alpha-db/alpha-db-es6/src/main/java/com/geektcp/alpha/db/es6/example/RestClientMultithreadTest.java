package com.geektcp.alpha.db.es6.example;
...
public class RestClientMultithreadTest {
...
//                + "es-example.properties";
        String path = FileUtils.class.getResource("/" + "es-example.properties").getPath();
        path = path.replace("\\", "\\\\");
        ...
        //EsServerHost in es-example.properties must as ip1:port1,ip2:port2,ip3:port3....  eg:1.1.1.1:24100,2.2.2.2:24102,3.3.3.3:24148
...
//        String krb5ConfFile = System.getProperty("user.dir") + File.separator + "conf" + File.separator + "krb5.conf";
        String krb5ConfFile = FileUtils.class.getResource("/" + "krb5.conf").getPath();
...
//        String jaasPath = System.getProperty("user.dir") + File.separator + "conf" + File.separator + "jaas.conf";
        String jaasPath = FileUtils.class.getResource("/" + "jaas.conf").getPath();
    ...
    private static void queryData(RestClient restClientTest) {
        ...
        Map<String, String> params = Collections.singletonMap("pretty", "true");
        try {
            rsp = restClientTest.performRequest("GET", "/" + index + "/" + index + "/" + id, params);
            if(HttpStatus.SC_OK == rsp.getStatusLine().getStatusCode()) {
    ...
    private static void bulk(RestClient restClientTest) {
        StringEntity entity = null;
            Gson gson = new Gson();
        Long idNumber = 0L;
        Long oneCommit = 10L;//Please modify the number of index documents submitted in each loop according to actual needs.
        Long totalRecordNumber = 100L;//Please modify the total number of index documents that need to be submitted in this bulk request method according to actual needs.
        Long circleNumber = totalRecordNumber/oneCommit;

            Map<String,Object> esMap = new HashMap<String,Object>();
        String str = "{ \"index\" : { \"_index\" : \"" + index + "\", \"_type\" :  \"" + index + "\"} }";
    ...
    public  static class sendRequestThread implements Runnable {
        ...
        public void run() {
            ...
            try {
                restCientTh = getRestClient(getHostArray(esServerHost));
    ...
    public static void main(String[] args) throws Exception {
        ...
        initProperties();
        RestClient restCientThNew=null;
        restCientThNew = getRestClient(getHostArray(esServerHost));
        int threadNumber = 10;//Please modify the number of threads according to actual needs.
        int jobNumber = 100;//Please modify the number of tasks according to actual needs.

        try {
            if (exist(restCientThNew)) {
                delete(restCientThNew);
            }
            createIndexWithShardNum(restCientThNew);
