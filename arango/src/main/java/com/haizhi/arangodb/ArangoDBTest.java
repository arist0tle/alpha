...
public class ArangoDBTest {
	...
	@Parameters
	public static Collection<Builder> builders() {
		return Arrays.asList(new Builder().useProtocol(Protocol.VST),
			new Builder().useProtocol(Protocol.HTTP_JSON),
			new Builder().useProtocol(Protocol.HTTP_VPACK));
	}
	...
	public void authenticationFailPassword() {
		final ArangoDB arangoDB = new Builder().password("no").build();
		try {
			arangoDB.getVersion();
			fail();
		} catch (final ArangoDBException e) {
	...
	public void authenticationFailUser() {
		final ArangoDB arangoDB = new Builder().user("no").build();
		try {
			arangoDB.getVersion();
			fail();
		} catch (final ArangoDBException e) {
	...
	public void fallbackHost() {
		final ArangoDB arangoDB = new Builder().host("not-accessible", 8529).host("127.0.0.1", 8529).build();
		final ArangoDBVersion version = arangoDB.getVersion();
		assertThat(version, is(notNullValue()));
	}
	...
	public void loadproperties() {
		new Builder().loadProperties(ArangoDBTest.class.getResourceAsStream("/arangodb-bad.properties"));
	}
